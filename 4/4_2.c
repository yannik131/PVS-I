...
/**
 * g) 
 * 1. Jeder Prozess kann mit seinen Daten vor dem Synchronisieren alle bis auf 2 Einträge berechnen. Während des Wartens kann also bereits gerechnet werden. Schema: 
 *  - Schicke Randwerte an Nachbarn
 *  - Warte nicht-blockierend auf diese Randwerte (MPI_Irecv)
 *  - Berechne alle Einträge, die ohne die nötigen Randwerte berechnet werden können
 *  - Nach der Rechnung: Warte auf die Randwerte, falls sie immer noch nicht angekommen sind
 *  - Sobald die Randwerte angekommen sind, können die 2 verbliebenen Werte berechnet werden
 *  - Beginne von vorn
 * 2. Bezogen auf a) - f): MPI_Recv ist blockierend und es sind 2 Aufrufe nötig -> Nur mit einem gleichzeitig
 * 3. Für diese Aufgabe muss jeder Prozess zwei double-Werte schicken und empfangen. Mit 8 byte pro double werden np*8 byte verschickt - eine vernachlässigbare Datenmenge bei Übetragungsraten von GBit/s.
 * 4. Sei np die Anzahl der Prozesse. Dann muss jeder Prozess N = 512*1024*1024 / np Zellenwerte berechnen. Dauert das 0.3ms, dann dauert eine Berechnung (also ein Zeitschritt dt) 0.3ms / N. Insgesamt verstreichen 0.3ms + 0.1ms = 0.4ms. 0.3ms, also 3/4 = 75% der Zeit wird also gerechnet.
 */
...
void synchronize_borders(double *T_k, int grid_size, int block_size, int r,
    ...
    MPI_Recv(T_k + index_right_receive, 1, MPI_DOUBLE, right_neighbor_rank, 0,
    ...
    MPI_Recv(T_k + index_left_receive, 1, MPI_DOUBLE, left_neighbor_rank, 0,
             MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}
...
int main() {
    ...
    int block_end =
        block_size *
        (r +
         1); // Removed - 1 at the end, alternatively iterate to <= block_end
    // printf("%i: block_begin = %i, block_end = %i\n", r, block_begin,
    // block_end);

    // The original code assigned temperatures from 0K (impossible) to about 500
    // million K (temperature in the sun's core: ~15.6 million K)
    int max_T = 1000;
    int min_T = 273;
    double a = (max_T - min_T) / (grid_size - 1);
    double b = min_T;

    // Assign temperatures from 273K (room temperature) to 1000K (red glowing
    // iron)
    ...
    for (int k = 0; k < num_time_steps; ++k) {
        ...
        for (int i = block_begin; i < block_end; ++i) {
            ...
            double dTdt_i =
                conductivity * (-2 * T_k[i] + T_k[i_left] + T_k[i_right]);
